...
const getUploadedProfileImage = (req: Request) => {
  ...
  return file ? `/uploads/profiles/${file.filename}` : undefined;
};
...
export const authController = {
  ...
  whoami: async (req: AuthorizedRequest, res: Response, next: NextFunction) => {
    try {
  ...
  updateProfile: async (
    ...
    try {
      const image = getUploadedProfileImage(req);
