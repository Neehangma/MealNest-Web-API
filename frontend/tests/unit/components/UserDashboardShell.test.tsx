...
jest.mock("@/app/dashboard/user/_components/DashboardHeader", () =>
  function MockDashboardHeader({ user, onToggleSidebar }: { user: { fullName?: string }; onToggleSidebar: () => void }) {
    return <header><span>{user.fullName || "Guest"}</span><button onClick={onToggleSidebar}>Open navigation</button></header>;
  });
jest.mock("@/app/dashboard/user/_components/Sidebar", () =>
  function MockSidebar({ open, onNavigate }: { open: boolean; onNavigate: () => void }) {
    return <aside data-open={open}><button onClick={onNavigate}>Navigate</button></aside>;
  });
