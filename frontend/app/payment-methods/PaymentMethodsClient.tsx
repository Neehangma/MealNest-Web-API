        ...
        {showAddForm && <div className="payment-methods-form-card">
          ...
          <form ref={formRef} className="payment-methods-form" onSubmit={handleSubmit}>
            {selectedType === "esewa" ? <>
              ...
              <label><span>ESEWA ID</span><input name="esewaId" type="text" inputMode="numeric" maxLength={10} value={esewaId} onChange={(event) => { const value = sanitizeEsewaId(event.target.value); setEsewaId(value); if (isEsewaIdValid(value)) setFormMessage(""); }} placeholder="Enter 10-digit eSewa ID" required/></label>
